...
func ParseFile(source *Source) (*QueryDocument, *quantyerror.Error) {
	p := parser{
...
func (p *parser) parseQueryDocument() *QueryDocument {
	...
	for p.peek().Kind != lexer.EOF {
		...
		case lexer.Name:
			...
			case "import":
				doc.Imports = p.parseImportDefinition()
			case "fragment":
...
func (p *parser) parseImportDefinition() *ImportDefinition {

	var def ImportDefinition
	def.Position = p.peekPos()
	def.Operation = p.parseOperationType()
	def.Files = make(ImportFileList, 0)

	p.many(lexer.ParenL, lexer.ParenR, func() {
		def.Files = append(def.Files, p.parseImportFile())
	})
...
func (p *parser) parseImportFile() *ImportFileDefinition {
	...
	def.Namespace = p.parseName()
	def.Path = p.parseValueLiteral(false)
